...
source_data = Table.read('GasMLR_vs_DustMLR_withAccepted+BestFitLines.csv', format='ascii.csv')
...
plt.plot(np.sort(Gas_MassLoss_Rate), 0.005*np.sort(Gas_MassLoss_Rate), '--', markersize=6, color='dimgray') #1/200 (=0.005) line. y=0.005x
...
plt.savefig('GasMassLossRate_Vs_DustMassLossRate.png')
plt.show()
